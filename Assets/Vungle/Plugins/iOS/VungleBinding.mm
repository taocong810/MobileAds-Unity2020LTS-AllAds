...
BOOL _vungleIsSoundEnabled() {
    return ![VungleSDK sharedSDK].muted;
}
...
void _vunglePlayAd(char* opt, const char* placementID) {
    ...
    if([obj isKindOfClass:[NSDictionary class]]) {
        ...
        options[VunglePlayAdOptionKeyStartMuted] = @([VungleSDK sharedSDK].muted);
        if (from[@"userTag"])
...
BOOL _vungleIsBannerAvailable(const char* placementID, int size) {
    return [[VungleManager instance] isBannerAvailable:GetStringParam(placementID) withSize:size];
}

void _vungleLoadBanner(const char* placementID, int bannerSize, int bannerPosition) {
    [[VungleManager instance] requestBanner:GetStringParam(placementID) withSize:bannerSize atPosition:bannerPosition];
}

void _vungleSetOffset(const char* placementID, int x, int y) {
    [[VungleManager instance] setOffset:GetStringParam(placementID) x:x y:y];
}

void _vungleShowBanner(const char* placementID) {
    [[VungleManager instance] showBanner:GetStringParam(placementID)];
}

void _vungleCloseBanner(const char* placementID) {
    [[VungleManager instance] closeBanner:GetStringParam(placementID)];
}
