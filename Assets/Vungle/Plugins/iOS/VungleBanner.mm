...
- (void)showBanner:(NSString *)options {
    ...
    if (!succeed) {
        ...
        if (bannerSize == VunglePluginAdSizeBannerMrec && ![VungleSDK sharedSDK].muted) {
...
- (NSMutableDictionary *)parseOptions:(NSString *)options {
    ...
    if (options.length) {
        ...
        if (json[@"muted"]) {
            opt[VunglePlayAdOptionKeyStartMuted] = json[@"muted"];
        } else {
            opt[VunglePlayAdOptionKeyStartMuted] = @([VungleSDK sharedSDK].muted);
        }
    } else {
        opt[VunglePlayAdOptionKeyStartMuted] = @([VungleSDK sharedSDK].muted);
    }
