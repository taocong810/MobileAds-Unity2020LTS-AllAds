...
- (BOOL)showBanner:(NSError **)error {
    ...
    if (!succeed) {
        [VungleUtility sendLog:[NSString stringWithFormat:@"Failed to show banner %@ with error %@", placementID, [*error localizedDescription]]];
        [self closeBanner];
    }
    return succeed;
}
...
+ (VungleAdSize) getVungleBannerSize:(int)adSizeIndex {
    ...
    switch (adSizeIndex) {
        ...
        default:
            [VungleUtility sendWarningMessage:@"Unknown banner size provided."];
...
- (VungleBannerPosition) getVungleBannerPosition:(int)adSizeIndex {
    ...
    switch (adSizeIndex) {
        ...
        case 6:
            ...
            break;
        default:
            [VungleUtility sendWarningMessage:@"Unknown banner position provided."];
            position = Unknown;
            break;
    }
    return position;
...
- (CGSize)convertVunglePluginAdSizeToCGSize:(VunglePluginAdSize)size {
    ...
    switch (size) {
        ...
        case VunglePluginAdSizeBannerLeaderboard:
            ...
            break;
        default:
            [VungleUtility sendErrorMessage:@"Invalid ad size provided."];
            returnSize = CGSizeZero;
            break;
    }
    return returnSize;
        ...
        switch (adPosition) {
            case TopLeft:
                [constraints addObjectsFromArray:@[
                    firstConstraint = [adViewContainer.leftAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.leftAnchor constant:x],
                    secondConstraint = [adViewContainer.topAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.topAnchor constant:y]]];
                break;
            case TopCenter:
                [constraints addObjectsFromArray:@[
            ...
            case TopRight:
                [constraints addObjectsFromArray:@[
                    firstConstraint = [adViewContainer.rightAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.rightAnchor constant:x],
                    secondConstraint = [adViewContainer.topAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.topAnchor constant:y]]];
                break;
            case Centered:
                [constraints addObjectsFromArray:@[
            ...
            case BottomLeft:
                [constraints addObjectsFromArray:@[
                    firstConstraint = [adViewContainer.leftAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.leftAnchor constant:x],
                    secondConstraint = [adViewContainer.bottomAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.bottomAnchor constant:y]]];
                break;
            case BottomCenter:
                [constraints addObjectsFromArray:@[
                    firstConstraint = [adViewContainer.centerXAnchor constraintEqualToAnchor:superview.safeAreaLayoutGuide.centerXAnchor constant:x],
